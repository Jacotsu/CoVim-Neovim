        ...
        if 'packet_type' in packet.keys():
            ...
            if packet['packet_type'] == 'message':
                if data['message_type'] == 'error_newname_taken':
                    ...
                    vim_print(self.vim, 'ERROR: Name already in use. Please try a different name')
                if data['message_type'] == 'error_newname_invalid':
                    self.CoVim.disconnect()
                    vim_print(self.vim, 'ERROR: Name contains illegal characters. Only numbers, letters, underscores, and dashes allowed. Please try a different name')
                if data['message_type'] == 'connect_success':
                    self.CoVim.setupWorkspace()
                    if 'buffer' in data.keys():
                        self.CoVim.vim_buffer = data['buffer']
                        self.vim.current.buffer[:] = self.CoVim.vim_buffer
                    self.CoVim.addUsers(data['collaborators'])
                    vim_print(self.vim, 'Success! You\'re now connected [Port '+str(self.CoVim.port)+']')
                if data['message_type'] == 'user_connected':
                    self.CoVim.addUsers([data['user']])
                    vim_print(self.vim, data['user']['name']+' connected to this document')
                if data['message_type'] == 'user_disconnected':
                    self.CoVim.remUser(data['name'])
                    vim_print(self.vim, data['name']+' disconnected from this document')
                ...
                if 'updated_cursors' in data.keys():
                    ...
                    for updated_user in data['updated_cursors']:
                        if self.CoVim.username != updated_user['name']:
                            self.vim.command(':call matchdelete(' + str(self.CoVim.collab_manager.collaborators[updated_user['name']][1]) + ')')
                            self.vim.command(':call matchadd(\'' +
...
class CoVimFactory(ClientFactory):
    ...
    def clientConnectionLost(self, connector, reason):
        #THIS IS A HACK
        if hasattr(self.CoVim, 'buddylist'):
            self.CoVim.disconnect()
            vim_print(self.vim, 'Lost connection.')

    def clientConnectionFailed(self, connector, reason):
        self.CoVim.disconnect()
        vim_print(self.vim, 'Connection failed.')
...
class CoVimScope:
    ...
    def initiate(self,
        ...
        if not port and default_port != '0':
            ...
            raise ValueError('No port specified and default port is 0')
        if not name and default_name != '0':
            name = default_name
        else:
            raise ValueError('No name specified and default name is 0')
        ...
        if not addr or not port or not name:
            vim_print(self.vim, 'Syntax Error: Use form :CoVim connect <server address> <port> <name>')
            return
        port = int(port)
        addr = str(addr)
        ...
        if hasattr(self, 'buddylist'):
            ...
            for name in self.collab_manager.collaborators.keys():
                if name != self.username:
                    self.vim.command(':call matchdelete('+str(self.collab_manager.collaborators[name][1]) + ')')
            del(self.buddylist)
...
class Main(object):
    ...
    def cursor_update(self):
        if self.CoVim.reactor_thread:
            reactor.callFromThread(self.CoVim.fact.cursor_update)

    @neovim.autocmd('CursorMovedI')
    def buffer_update(self):
        if self.CoVim.reactor_thread:
            reactor.callFromThread(self.CoVim.fact.buff_update)
